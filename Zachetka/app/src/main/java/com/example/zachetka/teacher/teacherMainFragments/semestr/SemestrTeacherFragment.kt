package com.example.zachetka.teacher.teacherMainFragments.semestr
...
class SemestrTeacherFragment : Fragment() {
    ...
    lateinit var spinnerStudentsSem: Spinner
    lateinit var spinnerGroupsSem: Spinner
    var selectedAttribute: String? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {

        v = inflater.inflate(R.layout.fragment_semestr_teacher, container, false)

        linRet = v.findViewById(R.id.linearRatingsST)
        tableRet = v.findViewById(R.id.ratingsST)

        spinnerStudentsSem = v.findViewById(R.id.spinnerStudentsSem)
        spinnerGroupsSem = v.findViewById(R.id.spinnerGroupsSem)
        ...
        spinnerStudentsSem.adapter = adapterUsers1
        cursorUsers1.close()
        ...
        spinnerGroupsSem.adapter = adapterGroups1
        cursorGroups1.close()

        var studentName: String = spinnerStudentsSem.selectedItem.toString()
        var groupsName: String = spinnerGroupsSem.selectedItem.toString()
        ...
        spinnerStudentsSem.adapter = adapterUsers
        cursorUsers.close()

        val layoutParams = TableRow.LayoutParams(0, TableRow.LayoutParams.WRAP_CONTENT, 1f)
        layoutParams.weight = 1f

        spinnerStudentsSem.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                ...
                studentName = spinnerStudentsSem.selectedItem.toString()
                groupsName = spinnerGroupsSem.selectedItem.toString()

                val cursor = database.rawQuery("SELECT Discipline.nameDis, SemestrAttestation.semestr, SemestrAttestation.grade FROM SemestrAttestation, Discipline, Students WHERE SemestrAttestation.idDiscipline = Discipline.idDiscipline AND Students.idStudent = SemestrAttestation.idStudent  AND Students.idUser = Users.idUser AND Users.surname = '" + studentName.substring(0, studentName.length - 5) + "' AND Groups.idGroup = Students.idGroup AND Groups.title = '"+ groupsName.substring(0, groupsName.length - 1) + "'", null)
        ...
        spinnerGroupsSem.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                val selectedItem = parent.getItemAtPosition(position).toString()
